...
class StoreApiController extends Controller
{
    ...
    public function saveCoupon(Request $request, string $id): JsonResponse
    {
        ...
        $request->validate(['code' => ['required', 'string', 'max:80'], 'type' => ['required', 'in:percent,fixed,free_shipping'], 'value' => ['required', 'numeric'], 'expiresAt' => ['required', 'date'], 'usageLimit' => ['nullable', 'integer', 'min:0'], 'categoryIds' => ['nullable', 'array'], 'productIds' => ['nullable', 'array']]);
    ...
    public function createOrder(Request $request): JsonResponse
    {
        ...
        validator($order, ['id' => ['required', 'string', 'max:80'], 'customerName' => ['required', 'string'], 'customerEmail' => ['required', 'email'], 'fulfillment' => ['required', 'in:delivery,pickup'], 'address' => ['required_if:fulfillment,delivery', 'nullable', 'string'], 'lines' => ['required', 'array', 'min:1']])->validate();
        $order['address'] = $order['fulfillment'] === 'pickup' ? (string) ($order['address'] ?? '') : $order['address'];
        ...
        $lines = collect($order['lines'])->map(function (array $line) use ($productRows) {
            ...
            return ['productId' => $product->id, 'color' => (string) ($line['color'] ?? ''), 'size' => (string) ($line['size'] ?? ''), 'quantity' => (int) $line['quantity'], 'name' => $product->name, 'price' => (float) $product->price, 'image' => $product->image, 'brand' => $product->brand, 'categoryId' => $product->categoryId];
        })->all();
        ...
        DB::transaction(function () use ($order, $user) {
            DB::table('kitchen_orders')->updateOrInsert(['id' => $order['id']], [
                'user_id' => $user->id, 'createdAt' => now(), 'status' => $order['status'] ?? 'placed', 'customerName' => $order['customerName'], 'customerEmail' => $order['customerEmail'], 'address' => $order['address'], 'fulfillment' => $order['fulfillment'], 'subtotal' => $order['subtotal'], 'discount' => $order['discount'], 'shipping' => $order['shipping'], 'total' => $order['total'], 'couponCode' => $order['couponCode'] ?? null, 'campaignId' => $order['campaignId'] ?? null, 'updated_at' => now(), 'created_at' => now(),
            ]);
    ...
    private function productRow(object $row): array { return ['id' => $row->id, 'name' => $row->name, 'brand' => $row->brand, 'price' => (float) $row->price, 'compareAt' => $row->compareAt ? (float) $row->compareAt : null, 'categoryId' => $row->categoryId, 'image' => $row->image, 'gallery' => $this->decode($row->gallery), 'description' => $row->description, 'features' => $this->decode($row->features), 'stock' => (int) $row->stock, 'colors' => $this->decode($row->colors), 'sizes' => $this->decode($row->sizes ?? null), 'featured' => (bool) $row->featured, 'deal' => (bool) $row->deal, 'published' => (bool) $row->published]; }
    ...
    private function couponRow(object $row): array { return ['id' => $row->id, 'code' => $row->code, 'type' => $row->type, 'value' => (float) $row->value, 'minSpend' => (float) $row->minSpend, 'maxDiscount' => $row->maxDiscount ? (float) $row->maxDiscount : null, 'usageLimit' => (int) $row->usageLimit ?: null, 'uses' => (int) $row->uses, 'expiresAt' => (string) $row->expiresAt, 'active' => (bool) $row->active, 'categoryIds' => $this->decode($row->categoryIds), 'productIds' => $this->decode($row->productIds ?? null)]; }
    ...
    private function couponDiscount(string $code, array $lines, float $subtotal): array
    {
        ...
        if (!$coupon || !$coupon->active || now()->greaterThan($coupon->expiresAt) || $subtotal < (float) $coupon->minSpend) return ['coupon' => null, 'discount' => 0, 'freeShipping' => false];
        if ((int) $coupon->usageLimit > 0 && (int) $coupon->uses >= (int) $coupon->usageLimit) return ['coupon' => null, 'discount' => 0, 'freeShipping' => false];
        ...
        $productIds = $this->decode($coupon->productIds ?? null);
        if ($productIds && collect($lines)->contains(fn (array $line) => !in_array($line['productId'], $productIds, true))) return ['coupon' => null, 'discount' => 0, 'freeShipping' => false];
